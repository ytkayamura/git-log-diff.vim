function! git_log_diff#git_log_buffer#open(target = '.')
  ...
  if !empty(g:gitLogDiff.target_file)
    silent execute "read !git log --date=format-local:'\\%Y-\\%m-\\%d \\%H:\\%M' --pretty=format:'\\%h \\%cd \\%s \\%an \\%d' -- " . g:gitLogDiff.target_file
  else
    silent execute "read !git log --date=format-local:'\\%Y-\\%m-\\%d \\%H:\\%M' --pretty=format:'\\%h \\%cd \\%s \\%an \\%d'"
  endif
